...
class RecommendationService:
    ...
    def build_complementary_profile(
        ...
        *,
        ...
        preferred_threading = cls._normalize_text(
            cls._context_value(
                key="threading",
                anchor_products=anchor_products,
                attribute_filters=attribute_filters,
        ...
        preferred_gauge = cls._normalize_text(
            cls._context_value(
                key="gauge",
                anchor_products=anchor_products,
                attribute_filters=attribute_filters,
    ...
    def _clean_filter_map(cls, attribute_filters: Optional[Dict[str, str]]) -> Dict[str, str]:
        ...
        for key, value in dict(attribute_filters or {}).items():
            ...
            clean_value = cls._normalize_text(value)
        ...
        *,
        ...
        for candidate in list(anchor_products or [])[:3]:
            ...
            for key in cls._PROFILE_KEYS:
                normalized = cls._normalize_text(attrs.get(key))
                if not normalized:
                    continue
    ...
    def _complementary_adjustment(
        ...
        *,
        ...
        candidate_threading = cls._normalize_text(attrs.get("threading"))
        ...
        candidate_gauge = cls._normalize_text(attrs.get("gauge"))
    ...
    def _attribute_match_score(cls, *, actual: Any, expected: str, key: str) -> float:
        actual_text = cls._normalize_text(actual)
        expected_text = cls._normalize_text(expected)
    ...
    def _candidate_text(cls, candidate: Any) -> str:
        ...
        return cls._normalize_text(" ".join(str(chunk or "") for chunk in chunks if str(chunk or "").strip()))
